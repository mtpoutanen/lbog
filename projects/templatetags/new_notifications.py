...
class NewNotificationsNode(Node):
    ...
    def render(self, context):
        ...
        if user.id:
            # user.id = 0
            context['notifications'] = Notification.objects.filter(receiver=user.get_profile()).order_by('-time_created')[:self.num]
            context['new_notifications'] = Notification.objects.filter(receiver=user.get_profile(), seen=False)
        else:
